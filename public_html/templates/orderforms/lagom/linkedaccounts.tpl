{if ($linkableProviders || $hasLinkedProvidersEnabled) && $linkContext == 'linktable'}
    <table id="tableLinkedAccounts" class="table display w-100 data-driven m-b-0" data-ajax-url="{$linkedAccountsUrl}" data-on-draw-rebind-confirmation="true" data-lang-empty-table="{lang key='remoteAuthn.noLinkedAccounts'}">
        <thead>
...
{elseif $linkableProviders}
    ...
    <div class="social-login social-wide">
    ...
    {if $linkContext == 'checkout-new'}
        <div class="text-lighter text-center text-small text-divider text-divider-bottom"><span>{lang key='remoteAuthn.titleOr'}</span></div>
    {/if}
{/if}