...
{else}
        ...
        <div class="domain-pricing">
            ...
            <div class="tld-filters">
                ...
                <select multiple class="form-control custom-multiselect">
                    {foreach $categoriesWithCounts as $category => $count name=foo}
                        <option value="{$category}" {if $smarty.foreach.foo.first}selected{/if}>{lang key="domainTldCategory.$category" defaultValue=$category} ({$count})</option>                   
                    {/foreach}
                </select>
            </div>
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            {if ($lookupTerm && !$captchaError) || (!$captcha->isEnabled() && $lookupTerm)}
                {literal}
                    setTimeout(function(){
