{if isset($RSThemes.pages.serverstatus)}
    {include file=$RSThemes['pages']['serverstatus']['fullPath']}
{else}
    <div class="section network-status">
            ...
            <div class="d-flex">
                ...
                <div class="dropdown view-filter-btns">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        ...
                        {if $opencount > 0}
                            <li><a href="{$WEB_ROOT}/serverstatus.php?view=open"><span>{$LANG.networkissuesstatusopen}</span></a></li>                    
                        {/if}
                        {if $scheduledcount > 0}
                            <li><a href="{$WEB_ROOT}/serverstatus.php?view=scheduled"><span>{$LANG.networkissuesstatusscheduled}</span></a></li>
                        {/if}
                        {if $resolvedcount > 0}
                            <li><a href="{$WEB_ROOT}/serverstatus.php?view=resolved"><span>{$LANG.networkissuesstatusresolved}</span></a></li>
                        {/if}
                    </ul> 
                </div>
            </div>  
        ...
        <div class="network-status-body">
            {foreach from=$issues item=issue}
                <div class="panel">
                    ...
                    <div class="panel-body"> 
                        <div class="priority priority-{$issue.priority|lower}">
            ...
            {foreachelse}
                <div class="p-v-48">
                    {include file="$template/includes/message.tpl" type="success" msg=$noissuesmsg textcenter=true}
                </div>
            {/foreach}
        </div>
        {if $issues}
                                ...
                                <td class="text-center" id="uptime{$num}">
                                    ...
                                    function checkLsPort(num, port) {
                                        WHMCS.http.jqClient.post('serverstatus.php', 'ping=1&num=' + num + '&port=' + port, function(data) {
                                            var str = data;
                                            var res = str.replace("assets/img", "templates/lagom/assets/img/serverstatus");
