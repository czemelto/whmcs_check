{if isset($RSThemes.pages.clientareasecurity)}
    {include file=$RSThemes['pages']['clientareasecurity']['fullPath']}
{else}  
        ...
        <ul class="nav nav-tabs">
            ...
            {if $showSsoSetting }
                <li {if !$linkableProviders}class="active"{/if}>
                    <a href="#single-sign-on" data-toggle="tab" aria-expanded="true">
            ...
            {if $showSsoSetting}
                <div class="tab-pane {if !$securityquestionsenabled && !$twoFactorAuthAvailable}active{/if}" id="single-sign-on">
                    ...
                    <form id="frmSingleSignOn">
                        ...
                        <label class="panel panel-switch m-w-288">
                            <div class="panel-body">
                                <span class="switch-label">{$LANG.sso.title}</span>
                                <span class="switch-loader loader" hidden>{include file="$template/includes/loader.tpl" classes="spinner-sm"}</span>
