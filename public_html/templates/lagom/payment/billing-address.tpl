<div class="form-group" id="billingAddressChoice">
    <h6 class="m-t-16">{$LANG.billingAddress}</h6>
    <div class="radio">
        <label class="radio-inline icheck-label billing-contact-0">
            <input class="icheck-control" type="radio" name="billingcontact" value="0" {if !$billingcontact || $billingcontact !=$client->billingContactId} checked{/if} > <strong class="name">{$client->fullName}</strong>
    ...
    {foreach $client->contacts()->orderBy('firstname', 'asc')->orderBy('lastname', 'asc')->get() as $contact} 
        <div class="radio">
            <label class="radio-inline icheck-label billing-contact-{$contact->id}">
                <input class="icheck-control" type="radio" name="billingcontact" value="{$contact->id}" {if $billingcontact==$contact->id || $contact->id == $client->billingContactId} checked{/if} > <strong class="name">{$contact->fullName}</strong>
